...
func updateErc721Info() error {
	...
	// 更新名称
	if len(name) > 0 {
		if err := ctx.PutState(erc721InfoMapName, "name", name); err != SUCCESS {
	...
	// 更新符号
	if len(symbol) > 0 {
		if err := ctx.PutState(erc721InfoMapName, "symbol", symbol); err != SUCCESS {
	...
	// 更新tokenURI
	if len(tokenURI) > 0 {
		if err := ctx.PutState(erc721InfoMapName, "tokenURI", tokenURI); err != SUCCESS {
...
func approve() {
	...
	// 保存授权信息
	if err := ctx.PutState(tokenApproveMapName, tokenId.ToString(), to); err != SUCCESS {
...
func getTokenLatestTxInfo() {
	...
	if err != SUCCESS || len(latestTxInfo) == 0 {
