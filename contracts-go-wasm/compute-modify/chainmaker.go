...
type SimContext interface {
	...
	Sha256(hashInput []byte) ([]byte, ResultCode)
}
...
func (s *SimContextCommonImpl) Sha256(hashInput []byte) ([]byte, ResultCode) {
...
// Sha256
func Sha256(hashInput []byte) ([]byte, ResultCode) {
	ec := NewEasyCodec()
	valueByte := make([]byte, 32)
	valuePtr := int32(uintptr(unsafe.Pointer(&valueByte[0])))
	ec.AddBytes("hashInput", hashInput)
	ec.AddInt32("value_ptr", valuePtr)
	b := ec.Marshal()
	code := sysCall(getRequestHeader(ContractMethodSha256), string(b))
	if code != int32(SUCCESS) {
		return []byte("Sha256 error"), ERROR
	}
...
func GetBytesFromChain(ec *EasyCodec, methodLen string, method string) ([]byte, ResultCode) {
	...
	b := ec.Marshal()
	// ## send req get len
	// 这一步传到wasmer中根据valuePtr会把对应内存中的变量（此处为valueLen）进行修改
