...
type exchange interface {
	...
	buyNow(tokenId, from, to, metadata string) protogo.Response // return "true","false"
...
func (e *ExchangeContract) InvokeContract(method string) protogo.Response {
	...
	case "buyNow":
		...
		metadata := string(args[paramMetaData])
		return e.buyNow(token, from, to, metadata)
	default:
		return Error("Invalid method" + method)
	}
}
func (e *ExchangeContract) buyNow(tokenId, from, to, metadata string) protogo.Response {
	// 添加白名单
	args := make(map[string][]byte)
	args["address"] = []byte(from + "," + to)
	resp := sdk.Instance.CallContract("identity", "addWriteList", args)
	if string(resp.Payload) != "add write list success" {
		sdk.Instance.Infof("[buyNow] addWriteList failed", string(resp.Payload))
		return Error("[buyNow] addWriteList failed")
	}
	...
	args["metadata"] = []byte(metadata)
	resp = sdk.Instance.CallContract("erc721", "mint", args)
	if string(resp.Payload) != "mint success" {
		sdk.Instance.Infof("[buyNow] mint failed", string(resp.Payload))
		return Error("[buyNow] mint failed")
	}

	//向所有调用者授权from的资产管理权限
	args = make(map[string][]byte)
	args["approvalFrom"] = []byte(from)
	resp = sdk.Instance.CallContract("erc721", "setApprovalForAll2", args)
	if string(resp.Payload) != "setApprovalForAll2 success" {
		sdk.Instance.Infof("[buyNow] setApprovalForAll2 failed]", string(resp.Payload))
		return Error("[buyNow] setApprovalForAll2 failed]")
	}
	...
	args["to"] = []byte(to)
	args["tokenId"] = []byte(tokenId)
	resp = sdk.Instance.CallContract("erc721", "transferFrom", args)
	if string(resp.Payload) != "transfer success" {
		return Error("erc721 transferFrom error. " + resp.Message)
	}
	...
	sdk.Instance.EmitEvent("buyNow", []string{from, to, spender, tokenId})

	return sdk.Success([]byte("buyNow success"))
}
