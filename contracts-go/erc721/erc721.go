...
type ERC721 interface {
	...
	setApprovalForAll2(operator string) protogo.Response
...
func (c *ERC721Contract) updateErc721Info() error {
	...
	sdk.Instance.Infof("[initinfo] name=" + string(name) + " symbol=" + string(symbol) + " tokenURI=" + string(tokenURI))
	if err != nil {
	...
	err = erc721Info.Set([]string{"admin"}, []byte(admin))
	if err != nil {
		return fmt.Errorf("set admin of erc721Info failed, err:%s", err)
	}
	return nil
}
...
func (c *ERC721Contract) InvokeContract(method string) protogo.Response {
	...
	case "mint":
		...
		return c.mint(metadata, to, tokenId, tokenIdStr)
	case "setApprovalForAll2":
		approvalFrom := string(args["approvalFrom"])
		if !address.IsValidAddress(approvalFrom) {
			return sdk.Error("invalid account")
		}
		return c.setApprovalForAll2(approvalFrom)
	default:
...
//这个 setApprovalForAll 函数的目的是 为第三方（operator）设置是否有权管理调用者（msg.sender）的所有资产
...
func (c *ERC721Contract) setApprovalForAll2(operator string) protogo.Response {
	sender, err := sdk.Instance.Sender()
	if err != nil {
		return sdk.Error(err.Error())
	}
	if sender == operator {
		return sdk.Success([]byte("ERC721: approve to caller"))
	}
	operatorApproveInfo, err := sdk.NewStoreMap(operatorApproveMapName, 2, crypto.HASH_TYPE_SHA256)
	if err != nil {
		return sdk.Error(fmt.Sprintf("new storemap of operatorApprove failed, err:%s", err))
	...
	approvedStr := trueString
	err = operatorApproveInfo.Set([]string{operator, sender}, []byte(approvedStr))
	if err != nil {
		return sdk.Error(fmt.Sprintf("set sender approve failed, err:%s", err))
	}
	sdk.Instance.EmitEvent("ApprovalForAll2", []string{operator, sender, approvedStr})

	return sdk.Success([]byte("setApprovalForAll2 success"))
}

//查看sender是否有控制owner资产的权限
func (c *ERC721Contract) isApprovedForAll(owner, sender string) protogo.Response {
	sdk.Instance.Infof("[isApprovedOrOwner] owner=" + string(owner) + " sender=" + string(sender))
	operatorApproveInfo, err := sdk.NewStoreMap(operatorApproveMapName, 2, crypto.HASH_TYPE_SHA256)
	if err != nil {
		return sdk.Error(err.Error())
	}
...
func (c *ERC721Contract) name() protogo.Response {
	...
	sdk.Instance.Infof("[name] name=" + string(name))
...
func (c *ERC721Contract) mint(metadata []byte, to string, tokenId *safemath.SafeUint256, originTokenId string) protogo.Response {
	...
	if minted {
		return sdk.Error(fmt.Sprintf("%s %v duplicated token", originTokenId, tokenId))
	}
	sender, err := sdk.Instance.Sender()
	if err != nil {
		return sdk.Error(fmt.Sprintf("get origin failed, err:%s", sender))
	}
	erc721Info, err := sdk.NewStoreMap(erc721InfoMapName, 1, crypto.HASH_TYPE_SHA256)
	if err != nil {
		return sdk.Error(fmt.Sprintf("new erc721Info map failed, err:%s", err))
	}
	admin, err := erc721Info.Get([]string{"admin"})
	_ = admin
	if err != nil {
		return sdk.Error(fmt.Sprintf("get admin from erc721Info failed, err:%s", err))
	}
	//删除这个限制
